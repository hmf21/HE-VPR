...
def get_backbone(backbone_arch='resnet50',
    ...
    elif 'mixfeat' in backbone_arch.lower():
        return backbones.MixFeat(backbone_arch)

    elif 'dinov2' in backbone_arch.lower():
        return backbones.DINOv2(model_name=backbone_arch, **backbone_config)

    elif 'crica_vit' in backbone_arch.lower():
        # with default dinov2 params
        return backbones.get_crica_backbone(pretrained_foundation=True)

    elif 'mona_vit' in backbone_arch.lower():
        # with default dinov2 params
...
def get_aggregator(agg_arch='ConvAP', agg_config={}):
    ...
    elif 'g_dino' in agg_arch.lower():
        if agg_config == {}:
            agg_config['p'] = 3
        else:
            assert 'p' in agg_config
        return aggregators.GeMPoolDino(**agg_config)
    ...
    elif 'he_salad' in agg_arch.lower():
        assert 'num_channels' in agg_config
        assert 'num_clusters' in agg_config
        assert 'cluster_dim' in agg_config
        assert 'token_dim' in agg_config
        return aggregators.HE_SALAD(**agg_config)

    elif 'crica' in agg_arch.lower():
        return aggregators.Crica(**agg_config)

    elif 'transvpr' in agg_arch.lower():
        transvpr_pool = aggregators.POOL()
        return transvpr_pool

...
if __name__ == '__main__':
    ...
    model = VPRModel(
        ...
        agg_arch='he_salad',
        agg_config={
            'num_channels': dino_feat_dim[dino_name],
            'num_clusters': 64,
            'cluster_dim': 128,
            'token_dim': 256,
        }
    ...
    ckpt_path = './weights-new/VPR_Mona/VPR_best.ckpt'   # with mask best
    ...
    if 'state_dict' in state_dict:
        ...
        for pretrained_key_index, pretrained_key in enumerate(state_dict['model_state_dict'].items()):
            ...
            if to_load_param_shape == pretrained_key[1].shape:
                model.state_dict()[to_load_key] = state_dict['model_state_dict'][pretrained_key[0]]
            else:
    ...
    for test_dataset in [test_datasets_batch[0], test_datasets_batch[2]]:

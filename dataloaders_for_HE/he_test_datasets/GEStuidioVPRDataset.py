...
dataset_dir = './GEStuidioVPRDataset'



class GEStuidioVPRDataset(data.Dataset):
    def __init__(self, img_rz=(256, 256)):
        ...
        self.test_db_dir = join(dataset_dir, 'map_database')
        ...
        self.qr_utms = np.array([(path.split("@")[3], path.split("@")[2]) for path in self.qr_paths]).astype(float)
        ...
        self.query_positive = 0.001

        self.dataset_info = {
            'num_db': self.numDb,
            'num_qr': self.numQ,
            'dataset_name': 'GStuidioVPRDataset',
...
if __name__ == '__main__':
    test_dataset = GEStuidioVPRDataset()
